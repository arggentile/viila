...
class FacturaAfipService {
	...
	{
            if(empty($nroPtoVta)){
                $this->error.=' INGRESE EL PUNTO VENTA PARA GENERAR LA FACTURA';
                $this->conerror= true;
            }else{
                $ptovta =  $nroPtoVta; 
                $wsaa = new WSAA('wsfe', $ptovta);
                if($wsaa->huboerror){
                   $this->error.=' ** '. $wsaa->error;
                   $this->error.=' ** NO SE ENCONTRARON LOS ARCHIVOS NECESARIOS PARA EL PTO VTA SELECCIONADO';
                   $this->conerror= true;             
                }else{
                   $this->ptoVta = $ptovta;
              }
            ...
            if($wsfev1->huboerror){
                ...
                $cae = $wsfev1->FECAESolicitar( 
                    ...
                    if(($cae === -1) || ($wsfev1->huboerror)) {
                        $this->error.= $wsfev1->error . '<br>Error al obtener CAE<br>';
                        $this->conerror= true;
                    }else{
                        $this->nroCae = $cae['cae'];
